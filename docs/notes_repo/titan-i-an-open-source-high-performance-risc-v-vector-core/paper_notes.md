...
- **工作流程**:
    ...
    1. **数据准备**：各 lane 在完成本地预处理后，将其数据块送入 Permutation Unit 的 **incoming queue**（输入队列）。
    1. **执行**：Permutation Unit 根据指令类型，执行 crossbar 交换或内部计算。
    1. **写回**：结果数据通过专用通道广播回各 lane，lane 可选择在 **VRF write-back stage** 等待响应。
...
- **算法流程**:
    ...
    1. 查询 CRT，检查是否存在地址冲突。
    1. 若无冲突，则更新 CRT 并将请求发送至 BIU；若有冲突，则等待。
    1. 当 BIU 完成一笔事务后，CRT 中对应的冲突区域被释放。
...
- **算法流程**:
    ...
    1. 检查 CSR chicken bit。若已设置，则标量核解除对该指令后续指令的阻塞。
    1. 编译器/软件将不相关的独立指令调度到该 indexed 指令之后。
    1. HOLSU 在后台处理所有内存事务，而前端继续执行 delay slot 中的指令。
