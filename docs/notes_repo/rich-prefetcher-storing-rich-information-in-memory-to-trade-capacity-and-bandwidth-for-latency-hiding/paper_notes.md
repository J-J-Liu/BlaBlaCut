...
- **输入与输出关系**：
  ...
  - **处理流程**：
    ...
    2. **触发判定**：当FIFO被填满（对于16KB/4KB）或发生单次访问（对于2KB）时，生成对应的触发事件。
    3. **模式查询**：使用触发事件（如哈希后的(PC, 5 offsets)）查询片上**Pattern History Table **(PHT)。
    4. **区域仲裁**：若多个区域同时触发，则根据预设的**固定优先级**进行仲裁。
...
- **仲裁优先级规则**（从高到低）：
  ...
  2. **16KB区域**（由 (PC, 5 offsets) 触发）
  3. **4KB区域**（由 (PC, 3 offsets) 触发）
  4. **2KB区域**（由 (PC, offset) 触发）
...
- **处理逻辑**：
    ...
    2. **重叠检查**：接着，将这个获胜请求与 **Inflight Prefetch Table**（飞行中预取表）进行比对。该表记录了所有正在进行中的预取操作所覆盖的内存区域。
